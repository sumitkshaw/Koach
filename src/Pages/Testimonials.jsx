...
export default function Testimonials() {
  ...
  const getVisibleIndices = () => {
   ...
   const getPositionClass = (index) => {
    ...
    if (index === centerIndex) {
      ...
      } else if (index === leftIndex1) {
        return "left-[40%] -translate-x-1/2 z-20 scale-105 ";
      } else if (index === leftIndex2) {
        return "left-[30%] -translate-x-1/2 z-10 scale-105 ";
      } else if (index === rightIndex1) {
        return "left-[60%] -translate-x-1/2 z-20 scale-105";
      } else if (index === rightIndex2) {
        return "left-[70%] -translate-x-1/2 z-10 scale-105";
      }
    return "hidden";
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto   py-5">
        <div className='bg-[#050A30] rounded-3xl  p-5'>
            <div className='text-white text-[36px] font-bold ml-64 text-left'>#1 Go-To Coaching Platform<br></br>
            <span className=''>for Next Gen Coaches, Startups, and VCs</span></div>
        </div>
        ...
        {testimonials.map((testimonial, index) => (
          <div
            ...
            className={`
                ...
                h-52 w-56 p-4 rounded-xl shadow-xl bg-white
      ...
      <button
        ...
        className="absolute left-36 top-1/2 -translate-y-1/2 z-50 rotate-180 rounded-full p-2  focus:outline-none focus:ring-2 focus:ring-[#050A30] "
      ...
      <button
        onClick={goToNext}
        className="absolute right-36 top-1/2 -translate-y-1/2 z-50 rounded-full p-2 focus:outline-none focus:ring-2 focus:ring-[#050A30]"
        disabled={isAnimating}
      >
        <img src={arrow} className="h-10 w-10" />
      </button>
   ...
   <style jsx>{`
        ...
        @keyframes slideToCenter {
          from { transform: translate(var(--from-x), -50%); opacity: 0.7; }
