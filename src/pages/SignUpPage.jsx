...
import { createUserWithEmailAndPassword, signInWithPopup, signInWithEmailAndPassword } from "firebase/auth"; // ✅ MOVE IT HERE
...
export default function SignUpPage() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const checkRes = await axios.get(`http://localhost:5001/api/onboarding/check?email=${user.email}`);
      const { exists, completed } = checkRes.data;

      if (exists && completed) {
        setError("User already signed up completely.");
      } else {
      ...
      // If sign-in fails → means user doesn't exist → so create them
      if (signInErr.code === "auth/user-not-found") {
        try {
          ...
          await axios.post("http://localhost:5001/api/onboarding/save", {
            step: 1,
            data: { email: user.email, uid: user.uid, profileUrl: user.photoURL }
          });

          navigate("/welcome");
        } catch (createErr) {
          console.error("❌ Signup failed:", createErr);
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="space-y-4 mb-6">
            ...
            <button onClick={handleSignUp} className="w-full h-12 bg-[#F5E649] text-[#000080] font-bold text-lg rounded-md shadow-md hover:bg-[#f3e22d] transition-colors">
              Sign up
            </button>
          </div>
          ...
          <div className="text-center mb-4">
            <p className="text-[#090E1C] text-base">
              Already have an account? <a onClick={()=>{navigate("/login")}} className="text-[#2D488F] hover:cursor-pointer">Log in</a>
            </p>
          </div>
